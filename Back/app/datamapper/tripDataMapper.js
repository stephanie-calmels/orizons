...
const tripDataMapper = {
        ...
        async createTrip(newTrip) {
                ...
                await client.query(`INSERT INTO "_m2m_trip_country"("trip_id", "country_id") VALUES $1, $2`, [trip.result[0].id, country.result[0].id]);
    ...
    async deleteOneTrip(tripId) {
        ...
        await client.query(`
                            DELETE FROM _m2m_trip_localisation WHERE trip_id = $1 `, [tripId]);
        await client.query(`
                            DELETE FROM _m2m_trip_category WHERE trip_id = $1 `, [tripId]);
        await client.query(`
