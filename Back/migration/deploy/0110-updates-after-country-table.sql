...
ALTER TABLE "step" ADD COLUMN "country_id" INT REFERENCES "country"("id") ;
...
SELECT s."id" AS "step_id", s."longitude", s."latitude", s."title", s."number_step", s."content", s."trip_id",
    ...
    JSON_AGG (DISTINCT "photo") AS "photos"
FROM "step" s
LEFT OUTER JOIN "country" ON "country"."id" = s."country_id"
LEFT OUTER JOIN "photo" ON "photo"."step_id" = s."id"
